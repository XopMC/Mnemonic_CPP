...
static void md_pad_128_swap(uint64_t* msg, const long msgLen_bytes) {
	uint32_t padLongIndex, overhang;
	padLongIndex = ((uint32_t)msgLen_bytes) / 8; 	overhang = (((uint32_t)msgLen_bytes) - padLongIndex * 8); 	msg[padLongIndex] &= SWAP512(maskLong[overhang]); 	msg[padLongIndex] |= SWAP512(padLong[overhang]); 	msg[padLongIndex + 1] = 0; 	msg[padLongIndex + 2] = 0; 	uint32_t i = 0;

		for (i = padLongIndex + 3; i < 32; i++) {
		msg[i] = 0;
	}
			msg[i - 2] = 0; 	msg[i - 1] = msgLen_bytes * 8; 		};
...
#define SECP256K1_FE_CONST_INNER(d7, d6, d5, d4, d3, d2, d1, d0) { (d0) & 0x3FFFFFFUL, (((std::uint32_t)d0) >> 26) | (((std::uint32_t)(d1) & 0xFFFFFUL) << 6), (((std::uint32_t)d1) >> 20) | (((std::uint32_t)(d2) & 0x3FFFUL) << 12), (((std::uint32_t)d2) >> 14) | (((std::uint32_t)(d3) & 0xFFUL) << 18), (((std::uint32_t)d3) >> 8) | (((std::uint32_t)(d4) & 0x3UL) << 24), (((std::uint32_t)d4) >> 2) & 0x3FFFFFFUL,(((std::uint32_t)d4) >> 28) | (((std::uint32_t)(d5) & 0x3FFFFFUL) << 4), (((std::uint32_t)d5) >> 22) | (((std::uint32_t)(d6) & 0xFFFFUL) << 10), (((std::uint32_t)d6) >> 16) | (((std::uint32_t)(d7) & 0x3FFUL) << 16), (((std::uint32_t)d7) >> 10) }
#define SECP256K1_SCALAR_CONST(d7, d6, d5, d4, d3, d2, d1, d0) {{(d0), (d1), (d2), (d3), (d4), (d5), (d6), (d7)}}
#define SECP256K1_FE_CONST(d7, d6, d5, d4, d3, d2, d1, d0) {SECP256K1_FE_CONST_INNER((d7), (d6), (d5), (d4), (d3), (d2), (d1), (d0))}
#define SECP256K1_GE_CONST(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) {SECP256K1_FE_CONST((a),(b),(c),(d),(e),(f),(g),(h)), SECP256K1_FE_CONST((i),(j),(k),(l),(m),(n),(o),(p)), 0}
#define SECP256K1_GE_CONST_INFINITY {SECP256K1_FE_CONST(0, 0, 0, 0, 0, 0, 0, 0), SECP256K1_FE_CONST(0, 0, 0, 0, 0, 0, 0, 0), 1}
#define SECP256K1_N_0 ((std::uint32_t)0xD0364141UL)
#define SECP256K1_N_1 ((std::uint32_t)0xBFD25E8CUL)
#define SECP256K1_N_2 ((std::uint32_t)0xAF48A03BUL)
#define SECP256K1_N_3 ((std::uint32_t)0xBAAEDCE6UL)
#define SECP256K1_N_4 ((std::uint32_t)0xFFFFFFFEUL)
#define SECP256K1_N_5 ((std::uint32_t)0xFFFFFFFFUL)
#define SECP256K1_N_6 ((std::uint32_t)0xFFFFFFFFUL)
#define SECP256K1_N_7 ((std::uint32_t)0xFFFFFFFFUL)
...
const secp256k1_ge_storage prec[128][4] = { {
...
static int secp256k1_scalar_reduce(secp256k1_scalar* r, std::uint32_t overflow) {
	uint64_t t;
...
static void secp256k1_scalar_set_b32(secp256k1_scalar* r, const uint8_t* b32, int* overflow) {
	int over;
	r->d[0] = (std::uint32_t)b32[31] | (std::uint32_t)b32[30] << 8 | (std::uint32_t)b32[29] << 16 | (std::uint32_t)b32[28] << 24;
	r->d[1] = (std::uint32_t)b32[27] | (std::uint32_t)b32[26] << 8 | (std::uint32_t)b32[25] << 16 | (std::uint32_t)b32[24] << 24;
	r->d[2] = (std::uint32_t)b32[23] | (std::uint32_t)b32[22] << 8 | (std::uint32_t)b32[21] << 16 | (std::uint32_t)b32[20] << 24;
	r->d[3] = (std::uint32_t)b32[19] | (std::uint32_t)b32[18] << 8 | (std::uint32_t)b32[17] << 16 | (std::uint32_t)b32[16] << 24;
	r->d[4] = (std::uint32_t)b32[15] | (std::uint32_t)b32[14] << 8 | (std::uint32_t)b32[13] << 16 | (std::uint32_t)b32[12] << 24;
	r->d[5] = (std::uint32_t)b32[11] | (std::uint32_t)b32[10] << 8 | (std::uint32_t)b32[9] << 16 | (std::uint32_t)b32[8] << 24;
	r->d[6] = (std::uint32_t)b32[7] | (std::uint32_t)b32[6] << 8 | (std::uint32_t)b32[5] << 16 | (std::uint32_t)b32[4] << 24;
	r->d[7] = (std::uint32_t)b32[3] | (std::uint32_t)b32[2] << 8 | (std::uint32_t)b32[1] << 16 | (std::uint32_t)b32[0] << 24;
...
static void secp256k1_scalar_cmov(secp256k1_scalar* r, const secp256k1_scalar* a, int flag) {
	std::uint32_t mask0, mask1;
	mask0 = flag + ~((std::uint32_t)0);
	mask1 = ~mask0;
...
static std::uint32_t secp256k1_scalar_get_bits(const secp256k1_scalar* a, std::uint32_t offset, std::uint32_t count) {
...
const  std::uint32_t M = 0x3FFFFFFUL, R0 = 0x3D10UL, R1 = 0x400UL;

static void secp256k1_fe_sqr_inner(std::uint32_t* r, const std::uint32_t* a) {
	uint64_t c, d;
...
static void secp256k1_fe_normalize_weak(secp256k1_fe* r) {
	std::uint32_t t0, t1, t2, t3, t4, t5, t6, t7, t8, t9;
	t0 = r->n[0];
	t1 = r->n[1];
	t2 = r->n[2];
	t3 = r->n[3];
...
static void secp256k1_fe_mul_inner(std::uint32_t* r, const std::uint32_t* a, const std::uint32_t* b) {
	uint64_t c, d;
	uint64_t u0, u1, u2, u3, u4, u5, u6, u7, u8;
	std::uint32_t t9, t1, t0, t2, t3, t4, t5, t6, t7;
		d = (uint64_t)a[0] * b[9]
		+ (uint64_t)a[1] * b[8]
		+ (uint64_t)a[2] * b[7]
		+ (uint64_t)a[3] * b[6]
...
static int secp256k1_fe_normalizes_to_zero(secp256k1_fe* r) {
	...
	std::uint32_t x = t9 >> 22; t9 &= 0x03FFFFFUL;
...
static void secp256k1_ecmult_gen(secp256k1_gej* r, secp256k1_scalar* gn) {
	...
	memset((uint8_t*)&adds, 0, (std::uint32_t)sizeof(adds));
...
static int secp256k1_fe_set_b32(secp256k1_fe* r, const uint8_t* a) {
	int ret;
	r->n[0] = (std::uint32_t)a[31] | ((std::uint32_t)a[30] << 8) | ((std::uint32_t)a[29] << 16) | ((std::uint32_t)(a[28] & 0x3) << 24);
	r->n[1] = (std::uint32_t)((a[28] >> 2) & 0x3f) | ((std::uint32_t)a[27] << 6) | ((std::uint32_t)a[26] << 14) | ((std::uint32_t)(a[25] & 0xf) << 22);
	r->n[2] = (std::uint32_t)((a[25] >> 4) & 0xf) | ((std::uint32_t)a[24] << 4) | ((std::uint32_t)a[23] << 12) | ((std::uint32_t)(a[22] & 0x3f) << 20);
	r->n[3] = (std::uint32_t)((a[22] >> 6) & 0x3) | ((std::uint32_t)a[21] << 2) | ((std::uint32_t)a[20] << 10) | ((std::uint32_t)a[19] << 18);
	r->n[4] = (std::uint32_t)a[18] | ((std::uint32_t)a[17] << 8) | ((std::uint32_t)a[16] << 16) | ((std::uint32_t)(a[15] & 0x3) << 24);
	r->n[5] = (std::uint32_t)((a[15] >> 2) & 0x3f) | ((std::uint32_t)a[14] << 6) | ((std::uint32_t)a[13] << 14) | ((std::uint32_t)(a[12] & 0xf) << 22);
	r->n[6] = (std::uint32_t)((a[12] >> 4) & 0xf) | ((std::uint32_t)a[11] << 4) | ((std::uint32_t)a[10] << 12) | ((std::uint32_t)(a[9] & 0x3f) << 20);
	r->n[7] = (std::uint32_t)((a[9] >> 6) & 0x3) | ((std::uint32_t)a[8] << 2) | ((std::uint32_t)a[7] << 10) | ((std::uint32_t)a[6] << 18);
	r->n[8] = (std::uint32_t)a[5] | ((std::uint32_t)a[4] << 8) | ((std::uint32_t)a[3] << 16) | ((std::uint32_t)(a[2] & 0x3) << 24);
	r->n[9] = (std::uint32_t)((a[2] >> 2) & 0x3f) | ((std::uint32_t)a[1] << 6) | ((std::uint32_t)a[0] << 14);
...
int secp256k1_ec_pubkey_serialize(uint8_t* output, std::uint32_t outputlen, const uint8_t* pubkey) {
	secp256k1_ge Q = { 0 };
	int ret = 0;
...
static void hash160(const uint8_t* input, int input_len, uint32_t* output) {
	uint8_t sha256_result[32];
	sha256((const uint32_t*)input, input_len, (uint32_t*)&sha256_result);
	ripemd160((const uint8_t*)&sha256_result, 32, output);
}

static void calc_uncompressed_hash160(extended_public_key_t* pub, uint32_t* hash160_bytes) {
	
	uint8_t serialized_pub_key[68] = { 0 };
	serialized_pub_key[0] = 04;
		for (int i = 0; i < 64; i++) {
		serialized_pub_key[i + 1] = pub->key[i];
	}
	hash160((const uint8_t*)&serialized_pub_key, 65, hash160_bytes);
}

static void calc_compressed_hash160(extended_public_key_t* pub, uint32_t* hash160_bytes) {

	uint8_t serialized_pub_key[36] = { 0 };
	serialized_public_key(pub, (uint8_t*)&serialized_pub_key);
	hash160((const uint8_t*)&serialized_pub_key, 33, hash160_bytes);
}


static void calc_segwit_hash160(extended_public_key_t* pub, uint32_t* hash160_bytes) {
	uint8_t serialized_pub_key[36] = { 0 };
	serialized_public_key(pub, (uint8_t*)&serialized_pub_key);
	uint8_t sha256_result[32];
...
static void calc_eth(uint32_t* pub, uint32_t* address_bytes) {
	ethhash h = { { 0 } };
	h.d[0] = pub[0];
	h.d[1] = pub[1];
	h.d[2] = pub[2];
...
static void hardened_private_child_from_private(const extended_private_key_t* parent, extended_private_key_t* child, uint32_t hardened_child_number) {
	...
	uint8_t hmac_input[40]; 	hmac_input[0] = 0;
	memcpy(&hmac_input[1], parent->key, 32);


	hardened_child_number |= 0x80000000;
...
static void normal_private_child_from_private(const extended_private_key_t* parent, extended_private_key_t* child, uint32_t normal_child_number) {
	...
	uint8_t hmac_input[40]; 	serialized_public_key(&pub, (uint8_t*)&hmac_input);
...
static void hardened_private_child_from_private_ed25519(const extended_private_key_t* parent, extended_private_key_t* child, uint32_t hardened_child_number) {
...
std::string hash160ToBase58(const uint8_t hash160[20], uint8_t prefix) {
	uint8_t extended[25];
	extended[0] = prefix;  	memcpy(&extended[1], hash160, 20);
	...
	return encodeBase58(extended, 25);*/
		...
		uint32_t extended32[7]; 	memset(extended32, 0, sizeof(extended32)); 	memcpy(extended32, extended, 21); 
	uint32_t hash[8]; 	sha256(extended32, 21, hash); 
		sha256(hash, 32, hash); 
		memcpy(&extended[21], hash, 4);
...
static bool bloom_chk_hash160(unsigned char* bloom, uint32_t* h) {
	unsigned int t;
...
#if defined(_WIN64) && !defined(__CYGWIN__) && !defined(__MINGW64__)
bool loadBloomFiltersIntoSharedMemory(const std::vector<std::string>& bloomFiles, unsigned char* blooms[100]) {
	...
	for (size_t i = 0; i < bloomFiles.size(); ++i) {
		const auto& bloomFile = bloomFiles[i];
			...
			0,
			BLOOM_SIZE,
			(L"BloomFilter_" + std::to_wstring(i)).c_str()
		);
...
bool loadBloomFiltersIntoSharedMemory(const std::vector<std::string>& bloomFiles, unsigned char* blooms[100]) {
	...
	for (size_t i = 0; i < bloomFiles.size(); ++i) {
		...
		if (mmap_ptr == MAP_FAILED) {
			std::cerr << "[!] Failed to mmap '" << bloomFile << "'. Error: " << strerror(errno) << std::endl;
			close(fd);
			return false;
		}

		blooms[i] = mmap_ptr; 		close(fd); 	}

	return true;
}
#endif

void PrintU(uint32_t* pub) {

	for (int i = 0; i < 8; i++) {
		uint32_t reversed = reverseBytes(pub[i]);
		std::cout << std::hex << std::setfill('0') << std::setw(8) << reversed;
	}
	cout << std::endl;
			}

typedef union hash160_u {
	uint8_t uc[RIPEMD160_DIGEST_LENGTH];
	uint32_t      ul[RIPEMD160_DIGEST_LENGTH >> 2];
} hash160_t;

typedef union xpoint_u {
	uint8_t uc[32];
	uint32_t      ul[32 >> 2];
} xpoint_t;
...
{
	for (int i = 0; i < blooms_count; i++)
	{
		if (bloom_chk_hash160(blooms[i], hash))
		{
			return true;
		}
	}
...
#if defined(_WIN64) && !defined(__CYGWIN__) && !defined(__MINGW64__)
static void check(hash160_t& hash, string value, string derivation, extended_private_key_t& PVK, string& mnemonic)
{
	if (find_in_bloom(hash.ul))
	{
		FOUND++;
		WaitForSingleObject(fileMutex, INFINITE); 		string privateKey;
		privateKey.resize(64);
		hex(PVK.key, 32, (unsigned char*)privateKey.data(), 64);
		string hash160;
		hash160.resize(40);
		hex(hash.uc, 20, (unsigned char*)hash160.data(), 40);
		std::ofstream out(outputFile, std::ios::app);
		if (!ADDRESSES) {
			if (out.is_open()) {
				out << mnemonic << ":" << derivation << ":" << privateKey << ":" << value << ":" << hash160;
				out << std::endl;
				out.close();
			}
			else {
				std::cerr << "Error: Unable to open the file: " << outputFile << std::endl;
			}
			WaitForSingleObject(printMutex, INFINITE);
			std::cerr << "[!] Found: " << mnemonic << ":" << derivation << ":" << privateKey << ":" << value << ":" << hash160 << std::endl;
		...
		else
		{
			if (value == "C")
			{
				string addr = hash160ToBase58(hash.uc, 0x00);
				char output[86] = { 0 };
				segwit_addr_encode(output, "bc", 0, hash.uc, sizeof(hash.uc));
				if (out.is_open()) {
					out << mnemonic << ":" << derivation << ":" << privateKey << ":" << addr;
					out << std::endl;
					out << mnemonic << ":" << derivation << ":" << privateKey << ":" << output;
					out << std::endl;
					out.close();
				}
				else {
				...
				WaitForSingleObject(printMutex, INFINITE);
				std::cerr << "[!] Found: " << mnemonic << ":" << derivation << ":" << privateKey << ":" << addr << std::endl;
				std::cerr << "[!] Found: " << mnemonic << ":" << derivation << ":" << privateKey << ":" << output << std::endl;
				ReleaseMutex(printMutex);
				ReleaseMutex(fileMutex); 
				...
				string addr = hash160ToBase58(hash.uc, 0x05);
				if (out.is_open()) {
					out << mnemonic << ":" << derivation << ":" << privateKey << ":" << addr;
					out << std::endl;
					out.close();
				...
				if (out.is_open()) {
					out << mnemonic << ":" << derivation << ":" << privateKey << ":0x" << hash160;
					out << std::endl;
					out.close();
				}
				else {
				...
				WaitForSingleObject(printMutex, INFINITE);
				std::cerr << "[!] Found: " << mnemonic << ":" << derivation << ":" << privateKey << ":0x" << hash160 << std::endl;
				ReleaseMutex(printMutex);
				ReleaseMutex(fileMutex); 
...
static void check_xpoint(xpoint_t& hash, string value, string derivation, extended_private_key_t& PVK, string& mnemonic)
{
	if (find_in_bloom(hash.ul))
	{
		FOUND++;
		...
		hex(PVK.key, 32, (unsigned char*)privateKey.data(), 64);
		string hash160;
		hash160.resize(64);		hex(hash.uc, 32, (unsigned char*)hash160.data(), 64);
		std::ofstream out(outputFile, std::ios::app);
		if (out.is_open()) {
			out << mnemonic << ":" << derivation << ":" << privateKey << ":" << value << ":" << hash160;
			out << std::endl;
...
static void check_solana(solana_t& hash, string value, string derivation, extended_private_key_t& PVK, string& mnemonic)
{
	if (find_in_bloom(hash.ul))
	{
		FOUND++;
		...
		if (!ADDRESSES) {
			...
			string addr = encodeBase58(hash.uc, 32);
			if (out.is_open()) {
				out << mnemonic << ":" << derivation << ":" << privateKey << ":" << value << ":" << addr;
				out << std::endl;
				out.close();
			}
			else {
			...
			WaitForSingleObject(printMutex, INFINITE);
			std::cerr << "[!] Found: " << mnemonic << ":" << derivation << ":" << privateKey << ":" << value << ":" << addr << std::endl;
			ReleaseMutex(printMutex);
			ReleaseMutex(fileMutex); 
...
static void check(hash160_t& hash, string value, string derivation, extended_private_key_t& PVK, string& mnemonic)
{
	...
	{
		FOUND++;
		pthread_mutex_lock(&fileMutex); 		string privateKey;
		privateKey.resize(64);
		hex(PVK.key, 32, (unsigned char*)privateKey.data(), 64);
		string hash160;
		hash160.resize(40);
...
static void key_to_hash160(
	const extended_private_key_t* master_private, const extended_private_key_t* solana_master_private, string& mnemonic
)
{
	hash160_t hash;
	...
	extended_private_key_t solana_start_key;
	extended_private_key_t solana_target_key;
	extended_private_key_t solana_target_key_fo_pub;

	std::string path_custom_copy = path_custom;

	size_t start = 0;
	size_t end = path_custom_copy.find("\n");
	uint32_t current_path = 0;
		
	while (end != std::string::npos)
	{
		std::string path = path_custom_copy.substr(start, end - start);
		if (!path.empty() && path.back() == '\r') {
			path.pop_back();
		}
		bool child_key_hardened = false;
		/*for (int i = 0; i < 32; ++i) {
			start_key.key[i] = master_private->key[i];
			start_key.chain_code[i] = master_private->chain_code[i];
		}*/
		memcpy(start_key.key, master_private->key, sizeof(master_private->key));
		memcpy(start_key.chain_code, master_private->chain_code, sizeof(master_private->chain_code));
		if (Solana)
		{
			memcpy(solana_start_key.key, solana_master_private->key, sizeof(solana_master_private->key));
			memcpy(solana_start_key.chain_code, solana_master_private->chain_code, sizeof(solana_master_private->chain_code));
		}
		...
		if (path == "m/" || path == "m")
		{
			...
			if (Ethereum)
			{
				calc_eth((uint32_t*)&target_public_key, hash.ul);
				check(hash, "E", path, start_key, mnemonic);
				totalHashes++;
			}
			if (Compressed)
			{
				calc_compressed_hash160(&target_public_key, hash.ul);
				check(hash, "C", path, start_key, mnemonic);
				totalHashes++;
			}
			if (Uncompressed) 
			{
				calc_uncompressed_hash160(&target_public_key, hash.ul);
				check(hash, "U", path, start_key, mnemonic);
				totalHashes++;
			}
			if (Segwit)
			{
				calc_segwit_hash160(&target_public_key, hash.ul);
				check(hash, "S", path, start_key, mnemonic);
				totalHashes++;
			}
			if (Xpoint) 
			{
		...
		/*	if (path.back() == '\'')
			...
			{
				result = 0;
				std::string token = path.substr(tokenStart, tokenEnd - tokenStart);
				if (token == "")
				{
					tokenStart = tokenEnd + 1;
					tokenEnd = path.find("/", tokenStart);
					continue;
				}
				tokenStart = tokenEnd + 1;
				tokenEnd = path.find("/", tokenStart);
				...
				{
					normal_private_child_from_private(&start_key, &target_key, result);
				}
				if (Solana) {
					hardened_private_child_from_private_ed25519(&solana_start_key, &solana_target_key, result);
					memcpy(solana_start_key.key, solana_target_key.key, sizeof(solana_target_key.key));
					memcpy(solana_start_key.chain_code, solana_target_key.chain_code, sizeof(solana_target_key.chain_code));
				}
								/*for (int i = 0; i < 32; ++i)
				{
					start_key.key[i] = target_key.key[i];
					start_key.chain_code[i] = target_key.chain_code[i];
				}*/
				memcpy(start_key.key, target_key.key, sizeof(target_key.key));
				memcpy(start_key.chain_code, target_key.chain_code, sizeof(target_key.chain_code));
			...
			string paths = path + to_string(result + i) + hard;
					...
					start_key.key[0], start_key.key[1], start_key.key[2], start_key.key[3],
					start_key.key[4], start_key.key[5], start_key.key[6], start_key.key[7],
					start_key.key[8], start_key.key[9], start_key.key[10], start_key.key[11],
					start_key.key[12], start_key.key[13], start_key.key[14], start_key.key[15],
					start_key.key[16], start_key.key[17], start_key.key[18], start_key.key[19],
					start_key.key[20], start_key.key[21], start_key.key[22], start_key.key[23],
					start_key.key[24], start_key.key[25], start_key.key[26], start_key.key[27],
					start_key.key[28], start_key.key[29], start_key.key[30], start_key.key[31]);*/

				if (child_key_hardened)
				{
					hardened_private_child_from_private(&start_key, &target_key, result + i);
				}
				else
				{
					normal_private_child_from_private(&start_key, &target_key, result + i);

				}
				if (Solana) {
					hardened_private_child_from_private_ed25519(&solana_start_key, &solana_target_key, result + i);
				...
				if (Solana)
				{
					...
					ed25519_create_keypair(solana.uc, seed, solana_target_key.key);
					check_solana(solana, "SOL", 'm' + paths, solana_target_key, mnemonic);
					totalHashes++;
				...
				if (Ethereum)
				{
					...
					/*printf("%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
						hash.uc[0], hash.uc[1], hash.uc[2], hash.uc[3], hash.uc[4],
						hash.uc[5], hash.uc[6], hash.uc[7], hash.uc[8], hash.uc[9],
				...
				/*	printf("%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
						...
						hash.uc[10], hash.uc[11], hash.uc[12], hash.uc[13], hash.uc[14],
						hash.uc[15], hash.uc[16], hash.uc[17], hash.uc[18], hash.uc[19]);*/
					check(hash, "C", "m" + paths, target_key, mnemonic);
					totalHashes++;
				}
				if (Uncompressed)
				{
				...
				if (Xpoint)
				{
					...
					xpoint_t xpoint;
					memcpy(xpoint.uc, &target_public_key.key[0], 32);
					check_xpoint(xpoint, "X", "m" + paths, target_key, mnemonic);
					totalHashes++;
				}
...
static void mnemonic_to_masterkey(string& mnemonic, uint8_t* hmac, uint8_t* hmac_sol) {

	size_t mnemonicLength = mnemonic.length();

	unsigned char salt[] = "mnemonic";
	size_t saltLength = sizeof(salt) - 1;
	unsigned char bitcoinSeed[] = "Bitcoin seed";
	unsigned char solanaSeed[] = "ed25519 seed";
	size_t bitcoinSeedLength = sizeof(bitcoinSeed) - 1;
	size_t solanaSeedLength = sizeof(solanaSeed) - 1;
			uint8_t seed[512 / 8];  
			PKCS5_PBKDF2_HMAC((char*)mnemonic.data(), mnemonic.length(), salt, saltLength, 2048, EVP_sha512(), 64, seed);

		if (isSecp256k1) {
		HMAC(EVP_sha512(), bitcoinSeed, bitcoinSeedLength, seed, 64, hmac, nullptr);
	}
	if (isEd25519)
	{
		
		HMAC(EVP_sha512(), solanaSeed, solanaSeedLength, seed, 64, hmac_sol, nullptr);
	}
	/*printf("Mnemo: %s len: %i\n", mnemonic.c_str(), mnemonic.length());
					printf("\n");
	printf("seed: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
	...
	printf("hmac: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
...
static bool Increment_byte(unsigned char* bytes, size_t length, int step) {
	...
	for (int i = length - 1; i >= 0; --i) {
		carry += bytes[i];
		bytes[i] = (unsigned char)(carry);
		if (bytes[i] != 0) {
			overflow = false;
		}
		carry >>= 8;
	}
	bool stop_local = !(!overflow || (carry == 0));
	if (stop_local && !stop.load()) {
		stop.store(true);
	}
...
static void GenerateMnemonic(const char* entropy, size_t entropy_size, std::string& mnemonic, const wchar_t* const* words) {
	...
	wstring mnemonic_phrase;
	uint8_t hash[32];
	sha256((uint32_t*)entropy, entropy_size, (uint32_t*)hash);
	std::vector<uint8_t> data((total_bits + 7) / 8, 0);
	memcpy(data.data(), entropy, entropy_size);

	for (size_t i = 0; i < checksum_size_bits; ++i) {
		if (hash[i / 8] & (1 << (7 - (i % 8)))) {
			data[(entropy_size * 8 + i) / 8] |= 1 << (7 - (i % 8));
		}
	}

	mnemonic.clear();  	for (size_t i = 0; i < word_count; ++i) {
		size_t index = 0;
		for (size_t j = 0; j < 11; ++j) {
			size_t bit_pos = i * 11 + j;
			if (data[bit_pos / 8] & (1 << (7 - (bit_pos % 8)))) {
				index |= 1 << (10 - j);
			}
		}

		mnemonic_phrase += words[index];
		if (i < word_count - 1) {
			mnemonic_phrase += L' ';  		}
	}
	mnemonic = ConvertToUTF8(mnemonic_phrase);
			}

#if defined(_WIN64) && !defined(__CYGWIN__) && !defined(__MINGW64__)
void workerThreadFile(std::istream& stream) {
	std::string line;
	if (!isHex)
	{
		uint8_t hmac[64];
...
void workerThread(std::istream& stream) {
	std::string line;
...
int main(int argc, char** argv) {
	...
	for (int arg = 1; arg < argc; ++arg) {
		...
		if ((strcmp(argv[arg], "-h") == 0) || (strcmp(argv[arg], "-help") == 0))
		{
			printf(
				...
				"-d FILE      \t\tRead derivations path's from FILE.\n"
				"-deep NUMBER \t\t0 - 2147483647 derivation deep.\n"
				"-c TYPES     \t\tUse TYPES for public key to hash160 computation.\n"
				"             \t\tMultiple can be specified. For example, the default\n"
				"             \t\tis 'cu', which checks for both uncompressed\n"
				...
				"             \t\tx - x coordinate\n"
				"             \t\tS - Solana address\n"
				"-custom FILE \t\tPath to your custom >=2048 words dict. Every word must be in new line.\n"
				"-in \t\t Allow program use your mnemonic or entropies from file or external input\n"
				"________________________________________________________________________________\n"
				"\t\t\t(WORKS WITH -in ONLY!!!)\t\t\t\n"
				"\t-entropy   \t\tTreat input as hex encoded entropy. (Can works with -custom and -lang)\n"
				"\t\t-step NUMBER  \t\tStep for incremental entropy brute (default: 1)\n"
				"\t\t-n NUMBER \tAdditional mode for -entropy FILE work with -step. This param add step for entropy n times\n"
				"\t-f FILE    \t\tRead mnemonics or entropies from FILE.\n"
				"________________________________________________________________________________\n"
				"-lang LANGUAGE \t\tLanguage for mnemonic from -entropy (don't work with -custom!!!). Example:\n"
				"\t\t-lang EN \tEnglish\n"
				"\t\t-lang SA \tSpanish\n"
				"\t\t-lang JA \tJapanese\n"
				"\t\t-lang IT \tItalian\n"
				...
				"-o FILE      \t\tWrite founds to FILE.\n"
				"-t NUMBER    \t\tNumber of threads.\n"
				"-save        \t\tSave founds like addresses\n"
				"-w NUMBER   \t\t(3 - 6 - 9 - 12 - 15 - 18 - 21 - 24 - 27 - 30 - 33 - 36 - 39 - 42 - 45 - 48...) Mnemonic words count for random gen.\n", (argv[0])
		...
		if (strcmp(argv[arg], "-c") == 0) {
			...
			for (size_t i = 0; i < strlen(value); ++i) {
				...
				case 'S':
					...
					break;
				default:
					std::cerr << "[!] Unknown value '" << value[i] << "' for '-c' option." << std::endl;
					break;
				}
			}
			arg++;
		}
		if (strcmp(argv[arg], "-custom") == 0) {
			if (arg + 1 >= argc) {
				std::cerr << "[!] Missing custom base file name after '-custom'." << std::endl;
				return 1;
			}
			cust = true;
			custom_file = argv[arg + 1];
			arg++;
		}
		if (strcmp(argv[arg], "-entropy") == 0) {
			israndom = false;
			isHex = true;
		}
		...
		if (strcmp(argv[arg], "-lang") == 0) {
			string language = argv[arg + 1];
			if ((language == "EN") || (language == "SA") || (language == "JA") || (language == "IT") || (language == "FR") || (language == "CZ") || (language == "PT") || (language == "KO") || (language == "CS") || (language == "CT")) {
				lang = language;
			}
			arg++;
		}
		if (strcmp(argv[arg], "-step") == 0) {
			if (arg + 1 >= argc) {
				std::cerr << "[!] Missing value after '-step'." << std::endl;
				return 1;
			}
			...
			try {
				...
				std::cerr << "[!] Error: Invalid value after '-step'. Setting to default value of 1." << std::endl;
				step = 1;
			}
		...
		if (strcmp(argv[arg], "-n") == 0) {
			if (arg + 1 >= argc) {
				std::cerr << "[!] Missing value after '-n'." << std::endl;
				return 1;
			}

			try {
				n = std::stoull(argv[arg + 1]);

			}
			catch (const std::exception& e) {
				std::cerr << "[!] Error: Invalid value after '-n'. Setting to default value of 0." << std::endl;
				n = 0;
			}
		...
		if (strcmp(argv[arg], "-d") == 0) {
			if (arg + 1 >= argc) {
				std::cerr << "[!] Missing derivation file name after '-d'." << std::endl;
				return 1;
			}
			isPath = true;
			derivationFile = argv[arg + 1];
			std::ifstream infile(derivationFile);
			if (!infile.is_open()) {
				std::cerr << "[!] Failed to open derivation file '" << derivationFile << "'" << std::endl;
				return 1;
			}
			path_custom = "";
			...
			if (path_custom == "" || path_custom.empty())
			{
				std::cerr << "[!] Empty derivation file '" << derivationFile << "'" << std::endl;
				return 1;
		...
		if (strcmp(argv[arg], "-deep") == 0) {
			if (arg + 1 >= argc) {
				std::cerr << "[!] Missing value after '-deep'." << std::endl;
				return 1;
			}

						long long value = std::stoll(argv[arg + 1]);

						if (value > 2147483647LL) {
				std::cerr << "[!] Warning: Value after '-deep' exceeds the maximum limit. Setting to default value of 5." << std::endl;
				num_childs = 5;
			}
			else {
				num_childs = static_cast<int>(value);
			}

			arg++; 		}
		if (strcmp(argv[arg], "-f") == 0) {
			if (arg + 1 >= argc) {
				std::cerr << "[!] Missing mnemonics file name after '-i'." << std::endl;
				return 1;
			}
		...
		if (strcmp(argv[arg], "-t") == 0) {
			...
			try {
				threads = std::stoi(argv[arg + 1]);

			}
			catch (const std::exception& e) {
				std::cerr << "[!] Error: Invalid value after '-t'. Setting to default value of 1." << std::endl;
				threads = 1;
			}
		...
		if (strcmp(argv[arg], "-w") == 0) {
			if (arg + 1 >= argc) {
				std::cerr << "[!] Missing value after '-w'." << std::endl;
				return 1;
			}

			try {
				words_count = std::stoi(argv[arg + 1]);
				while (words_count % 3 != 0 || words_count < 3) {
					words_count++;
				}
				entropy_len = (words_count / words_count_min) * entropy_count_min;

			}
			catch (const std::exception& e) {
				std::cerr << "[!] Error: Invalid value after '-w'. Setting to default value of 12." << std::endl;
				words_count = 1;
			}
	...
	if (!isPath) {
		cout << "[!] ERROR!!! Need to add derivation file with -d" << endl;
		return 1;
	}
	if (Ethereum || Compressed || Uncompressed || Segwit || Xpoint) { isSecp256k1 = true; }
	if (Solana) {
		isEd25519 = true;
	}
	if (israndom || isHex) {
		...
		if (cust)
		{
			...
			while (std::getline(file, line) && index < 2048) {
				Custom[index] = new wchar_t[line.size() + 1];
				wcscpy(Custom[index], line.c_str());
				index++;
			}

			file.close();
			current_dict = (const wchar_t**)Custom;
		}
	}
			std::cerr << "[!] Start Loading BloomFilters..." << endl;

	if (!loadBloomFiltersIntoSharedMemory(bloomFiles, blooms)) {
		std::cerr << "[!] Error while loading bloom filters into shared memory." << std::endl;
		return 1;
	}
	unsigned long long totalMnemonics1;
	unsigned long long totalHashes1;
	unsigned long long totalMnemonics2;
	unsigned long long totalHashes2;
	unsigned long long speedMnemonics;
	unsigned long long speedHashes;

#if defined(_WIN64) && !defined(__CYGWIN__) && !defined(__MINGW64__)
	fileMutex = CreateMutex(NULL, FALSE, NULL);
	printMutex = CreateMutex(NULL, FALSE, NULL);
	if (fileMutex == NULL) {
		std::cerr << "Error: Unable to create mutex." << std::endl;
		return 1;
	}
	if (!israndom) {
		if (usingFile) {
						
			std::ifstream stream(inputFile.c_str(), std::ios::binary);

			std::vector<std::thread> workers;
			for (int i = 0; i < threads; i++) {
				workers.push_back(std::thread(workerThreadFile, std::ref(stream)));
			}


			do
			{
				totalMnemonics1 = totalMnemonics;
				totalHashes1 = totalHashes;
				sleep_ms(1000); 				totalMnemonics2 = totalMnemonics;
				totalHashes2 = totalHashes;
				speedMnemonics = totalMnemonics2 - totalMnemonics1;
				speedHashes = totalHashes2 - totalHashes1;
				WaitForSingleObject(printMutex, INFINITE);
				printf("[!] Total mnemonics: %llu | Total hashes: %llu | Speed: %llumnem/s %lluhash/s | Found: %llu [!]\r", totalMnemonics, totalHashes, speedMnemonics, speedHashes, FOUND);
								ReleaseMutex(printMutex);
				fflush(stdout);
				sleep_ms(10000);
			} while (isRunning);
			...
			printf("[!] Total mnemonics: %llu | Total hashes: %llu | Speed: %llumnem/s %lluhash/s | Found: %llu [!]\n", totalMnemonics, totalHashes, speedMnemonics, speedHashes, FOUND);
		...
		else
		{
			...
			std::vector<std::thread> workers;
			for (int i = 0; i < threads; i++) {
				workers.push_back(std::thread(workerThread, std::ref(cin)));
			}
		...
		std::vector<std::thread> workers;
		for (int i = 0; i < threads; i++) {
			workers.push_back(std::thread(Entropy_Rand_Thread));
		}
	...
	if (!israndom) {
		if (usingFile) {
			pthread_t* workers = new pthread_t[threads];
									...
									std::ifstream stream(inputFile.c_str(), std::ios::binary);
			for (int i = 0; i < threads; i++) {
				pthread_create(&workers[i], NULL, workerThreadFile, &stream);
			}
			do
			{
				totalMnemonics1 = totalMnemonics;
		...
		else
		{
			...
			/*	InputStreamReaderThread InputReader;
				InputReader.start();*/

			for (int i = 0; i < threads; i++) {
				pthread_create(&workers[i], NULL, workerThread, &cin);
			}
			do
			{
				totalMnemonics1 = totalMnemonics;
		...
		for (int i = 0; i < threads; i++) {
			pthread_create(&workers[i], NULL, Entropy_Rand_Thread, NULL);
		}
		do
		{
			totalMnemonics1 = totalMnemonics;
...
#if defined(_WIN64) && !defined(__CYGWIN__) && !defined(__MINGW64__)
	for (size_t i = 0; i < bloomFiles.size(); ++i) {
		if (blooms[i] && !UnmapViewOfFile(blooms[i])) {
			std::cerr << "[!] Failed to unmap view of file for bloom " << i << ". Error code: " << GetLastError() << std::endl;
		}
	}
#else
	for (size_t i = 0; i < bloomFiles.size(); ++i) {
		if (blooms[i] && munmap(blooms[i], BLOOM_SIZE) == -1) {
			std::cerr << "[!] Failed to unmap '" << bloomFiles[i] << "'. Error: " << strerror(errno) << std::endl;
		}
	}
#endif
